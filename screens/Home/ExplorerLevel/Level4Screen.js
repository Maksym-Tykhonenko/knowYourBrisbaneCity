...
import {lvl4} from '../../../data/Explorer/lvl4';
...
const Level4Screen = ({navigation}) => {
  ...
  const [availableAnswers, setAvailableAnswers] = useState(
    lvl4[currentQuestionIndex].answers,
  ...
  const setCompliteData = async () => {
    try {
      ...
      const jsonData = JSON.stringify(data);
      await AsyncStorage.setItem(`Level4Screen`, jsonData);
      //console.log('Дані збережено в AsyncStorage');
    } catch (e) {
      console.log('Помилка збереження даних:', e);
    }
  ...
  const handleAnswerSelection = selectedAnswer => {
    ...
    if (selectedAnswer === currentQuestion.correctAnswer) {
      ...
      if (currentQuestionIndex < lvl4.length - 1) {
        setCurrentQuestionIndex(currentQuestionIndex + 1);
        setAvailableAnswers(lvl4[currentQuestionIndex + 1].answers); // Оновити варіанти відповідей
      } else {
  ...
  const GoToNext = () => {
    navigation.navigate('Level5Screen');
      ...
      <View style={styles.conteiner}>
        <ScrollView>
          <View style={{alignItems: 'center'}}>
            <Text style={styles.qwestion}>Tourism in Brisbane</Text>
            <View
              style={{
                width: windowWidth,
                flexDirection: 'row',
