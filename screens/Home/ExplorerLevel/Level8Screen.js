...
import {lvl8} from '../../../data/Explorer/lvl8';
...
const Level8Screen = ({navigation}) => {
  ...
  const [availableAnswers, setAvailableAnswers] = useState(
    lvl8[currentQuestionIndex].answers,
  ...
  const setCompliteData = async () => {
    try {
      ...
      const jsonData = JSON.stringify(data);
      await AsyncStorage.setItem(`Level8Screen`, jsonData);
      //console.log('Дані збережено в AsyncStorage');
    } catch (e) {
      console.log('Помилка збереження даних:', e);
    }
  ...
  const handleAnswerSelection = selectedAnswer => {
    ...
    const currentQuestion = lvl8[currentQuestionIndex];
    if (selectedAnswer === currentQuestion.correctAnswer) {
      // Якщо відповідь правильна, перейти до наступного питання
      setPoints(prevPoints => prevPoints + 10);
      if (currentQuestionIndex < lvl8.length - 1) {
        setCurrentQuestionIndex(currentQuestionIndex + 1);
        setAvailableAnswers(lvl8[currentQuestionIndex + 1].answers); // Оновити варіанти відповідей
      } else {
  ...
  const GoToNext = () => {
    navigation.navigate('Level9Screen');
      ...
      <View style={styles.conteiner}>
        <ScrollView>
          <View style={{alignItems: 'center'}}>
            <Text style={styles.qwestion}>Education in Brisbane</Text>
            <View
              style={{
                width: windowWidth,
                flexDirection: 'row',
