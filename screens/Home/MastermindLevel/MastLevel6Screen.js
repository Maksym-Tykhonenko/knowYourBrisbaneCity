...
import {lvl6Mastermind} from '../../../data/Mastermind/lvl6Mastermind';
...
const MastLevel6Screen = ({navigation, route}) => {
  ...
  const [availableAnswers, setAvailableAnswers] = useState(
    lvl6Mastermind[currentQuestionIndex].awnsers,
  ...
  const setCompliteData = async () => {
    try {
      ...
      const jsonData = JSON.stringify(data);
      await AsyncStorage.setItem(`LevelMastermind6Screen`, jsonData);
      //console.log('Дані збережено в AsyncStorage');
    } catch (e) {
      console.log('Помилка збереження даних:', e);
    }
  ...
  const handleAnswerSelection = selectedAnswer => {
    ...
    const currentQuestion = lvl6Mastermind[currentQuestionIndex];
    if (selectedAnswer === currentQuestion.correctAnswer) {
      // Якщо відповідь правильна, перейти до наступного питання
      setPoints(prevPoints => prevPoints + 10);
      if (currentQuestionIndex < lvl6Mastermind.length - 1) {
        setCurrentQuestionIndex(currentQuestionIndex + 1);
        setAvailableAnswers(lvl6Mastermind[currentQuestionIndex + 1].awnsers); // Оновити варіанти відповідей
      } else {
      ...
      <View style={styles.conteiner}>
        <ScrollView>
          <View style={{alignItems: 'center'}}>
            <Text style={styles.qwestion}>Brisbane’s Architecture</Text>
            <View
              style={{
                width: windowWidth,
                flexDirection: 'row',
...
export default MastLevel6Screen;
