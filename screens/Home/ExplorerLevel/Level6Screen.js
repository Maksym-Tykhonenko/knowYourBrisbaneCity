...
import {lvl6} from '../../../data/Explorer/lvl6';
...
const Level6Screen = ({navigation}) => {
  ...
  const [availableAnswers, setAvailableAnswers] = useState(
    lvl6[currentQuestionIndex].answers,
  ...
  const setCompliteData = async () => {
    try {
      ...
      const jsonData = JSON.stringify(data);
      await AsyncStorage.setItem(`Level6Screen`, jsonData);
      //console.log('Дані збережено в AsyncStorage');
    } catch (e) {
      console.log('Помилка збереження даних:', e);
    }
  ...
  const handleAnswerSelection = selectedAnswer => {
    ...
    const currentQuestion = lvl6[currentQuestionIndex];
    if (selectedAnswer === currentQuestion.correctAnswer) {
      // Якщо відповідь правильна, перейти до наступного питання
      setPoints(prevPoints => prevPoints + 10);
      if (currentQuestionIndex < lvl6.length - 1) {
        setCurrentQuestionIndex(currentQuestionIndex + 1);
        setAvailableAnswers(lvl6[currentQuestionIndex + 1].answers); // Оновити варіанти відповідей
      } else {
  ...
  const GoToNext = () => {
    navigation.navigate('Level7Screen');
      ...
      <View style={styles.conteiner}>
        <ScrollView>
          <View style={{alignItems: 'center'}}>
            <Text style={styles.qwestion}>Landmarks of Brisbane</Text>
            <View
              style={{
                width: windowWidth,
                flexDirection: 'row',
