...
import {lvl4Mastermind} from '../../../data/Mastermind/lvl4Mastermind';
...
const MastLevel4Screen = ({navigation, route}) => {
  ...
  const [availableAnswers, setAvailableAnswers] = useState(
    lvl4Mastermind[currentQuestionIndex].awnsers,
  ...
  const setCompliteData = async () => {
    try {
      ...
      const jsonData = JSON.stringify(data);
      await AsyncStorage.setItem(`LevelMastermind4Screen`, jsonData);
      //console.log('Дані збережено в AsyncStorage');
    } catch (e) {
      console.log('Помилка збереження даних:', e);
    }
  ...
  const handleAnswerSelection = selectedAnswer => {
    ...
    const currentQuestion = lvl4Mastermind[currentQuestionIndex];
    if (selectedAnswer === currentQuestion.correctAnswer) {
      // Якщо відповідь правильна, перейти до наступного питання
      setPoints(prevPoints => prevPoints + 10);
      if (currentQuestionIndex < lvl4Mastermind.length - 1) {
        setCurrentQuestionIndex(currentQuestionIndex + 1);
        setAvailableAnswers(lvl4Mastermind[currentQuestionIndex + 1].awnsers); // Оновити варіанти відповідей
      } else {
      ...
      <View style={styles.conteiner}>
        <ScrollView>
          <View style={{alignItems: 'center'}}>
            <Text style={styles.qwestion}>Brisbane’s Famous People</Text>
            <View
              style={{
                width: windowWidth,
                flexDirection: 'row',
...
export default MastLevel4Screen;
