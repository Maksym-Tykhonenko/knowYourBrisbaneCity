...
const Level10Screen = ({navigation}) => {
  ...
  const handleAnswerSelection = selectedAnswer => {
    ...
    if (selectedAnswer === currentQuestion.correctAnswer) {
      ...
      if (currentQuestionIndex < lvl10.length - 1) {
        setCurrentQuestionIndex(currentQuestionIndex + 1);
        setAvailableAnswers(lvl10[currentQuestionIndex + 1].answers); // Оновити варіанти відповідей
      } else {
      ...
      <View style={styles.conteiner}>
        <ScrollView>
          <View style={{alignItems: 'center'}}>
            <Text style={styles.qwestion}>Transportation in Brisbane</Text>
            <View
              style={{
                width: windowWidth,
                flexDirection: 'row',
