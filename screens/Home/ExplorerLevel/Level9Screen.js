...
import {lvl9} from '../../../data/Explorer/lvl9';
...
const Level9Screen = ({navigation}) => {
  ...
  const [availableAnswers, setAvailableAnswers] = useState(
    lvl9[currentQuestionIndex].answers,
  ...
  const setCompliteData = async () => {
    try {
      ...
      const jsonData = JSON.stringify(data);
      await AsyncStorage.setItem(`Level9Screen`, jsonData);
      //console.log('Дані збережено в AsyncStorage');
    } catch (e) {
      console.log('Помилка збереження даних:', e);
    }
  ...
  const handleAnswerSelection = selectedAnswer => {
    const currentQuestion = lvl9[currentQuestionIndex];
    if (selectedAnswer === currentQuestion.correctAnswer) {
      // Якщо відповідь правильна, перейти до наступного питання
      setPoints(prevPoints => prevPoints + 10);
      if (currentQuestionIndex < lvl9.length - 1) {
        setCurrentQuestionIndex(currentQuestionIndex + 1);
        setAvailableAnswers(lvl9[currentQuestionIndex + 1].answers); // Оновити варіанти відповідей
      } else {
  ...
  const GoToNext = () => {
    navigation.navigate('Level10Screen');
      ...
      <View style={styles.conteiner}>
        <ScrollView>
          <View style={{alignItems: 'center'}}>
            <Text style={styles.qwestion}>Economy of Brisbane</Text>
            <View
              style={{
                width: windowWidth,
                flexDirection: 'row',
