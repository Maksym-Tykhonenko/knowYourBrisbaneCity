...
import {lvl1Mastermind} from '../../../data/Mastermind/lvl1Mastermind';
...
const MastLevel1Screen = ({navigation}) => {
  ...
  const [availableAnswers, setAvailableAnswers] = useState(
    lvl1Mastermind[currentQuestionIndex].awnsers,
  ...
  const setCompliteData = async () => {
    try {
      ...
      const jsonData = JSON.stringify(data);
      await AsyncStorage.setItem(`LevelMastermind1Screen`, jsonData);
      //console.log('Дані збережено в AsyncStorage');
    } catch (e) {
      console.log('Помилка збереження даних:', e);
    }
  ...
  const handleAnswerSelection = selectedAnswer => {
    const currentQuestion = lvl1Mastermind[currentQuestionIndex];
    if (selectedAnswer === currentQuestion.correctAnswer) {
      // Якщо відповідь правильна, перейти до наступного питання
      setPoints(prevPoints => prevPoints + 10);
      if (currentQuestionIndex < lvl1Mastermind.length - 1) {
        setCurrentQuestionIndex(currentQuestionIndex + 1);
        setAvailableAnswers(lvl1Mastermind[currentQuestionIndex + 1].awnsers); // Оновити варіанти відповідей
      } else {
...
export default MastLevel1Screen;
