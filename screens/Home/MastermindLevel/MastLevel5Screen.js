...
import {lvl5Mastermind} from '../../../data/Mastermind/lvl5Mastermind';
...
const MastLevel5Screen = ({navigation, route}) => {
  ...
  const [availableAnswers, setAvailableAnswers] = useState(
    lvl5Mastermind[currentQuestionIndex].awnsers,
  ...
  const setCompliteData = async () => {
    try {
      ...
      const jsonData = JSON.stringify(data);
      await AsyncStorage.setItem(`LevelMastermind5Screen`, jsonData);
      //console.log('Дані збережено в AsyncStorage');
    } catch (e) {
      console.log('Помилка збереження даних:', e);
    }
  ...
  const handleAnswerSelection = selectedAnswer => {
    ...
    const currentQuestion = lvl5Mastermind[currentQuestionIndex];
    if (selectedAnswer === currentQuestion.correctAnswer) {
      // Якщо відповідь правильна, перейти до наступного питання
      setPoints(prevPoints => prevPoints + 10);
      if (currentQuestionIndex < lvl5Mastermind.length - 1) {
        setCurrentQuestionIndex(currentQuestionIndex + 1);
        setAvailableAnswers(lvl5Mastermind[currentQuestionIndex + 1].awnsers); // Оновити варіанти відповідей
      } else {
      ...
      <View style={styles.conteiner}>
        <ScrollView>
          <View style={{alignItems: 'center'}}>
            <Text style={styles.qwestion}>Brisbane’s Festivals</Text>
            <View
              style={{
                width: windowWidth,
                flexDirection: 'row',
...
export default MastLevel5Screen;
